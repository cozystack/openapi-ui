...
type TAllResourcesListProps = {
  namespace?: string
}

export const AllResourcesList: FC<TAllResourcesListProps> = ({ namespace }) => {
        ...
        {!apiGroupList.error && crdGroups && (
          <TreeWithSearch
            ...
            onSelect={(_, info) => {
              if (info.node.isLeaf) {
                navigate(
                  `${baseprefix}/${cluster}${namespace ? `/${namespace}` : ''}/crds-by-api/${
                    info.node.key
                  }/${apiExtensionVersion}`,
                )
              }
      ...
      <Styled.FullHeighCard title="API Groups">
        ...
        {!apiGroupList.error && nonCrdGroups && (
          <TreeWithSearch
            ...
            onSelect={(_, info) => {
              if (info.node.isLeaf) {
                navigate(`${baseprefix}/${cluster}${namespace ? `/${namespace}` : ''}/apis-by-api/${info.node.key}`)
              }
            }}
          />
        )}
